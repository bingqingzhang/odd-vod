# dataset settings
dataset_type = 'ImagenetVIDDatasetODDs'
...
test_pipeline = [
    ...
    dict(
        type='VideoCollect',
        keys=['img'],
        meta_keys=('num_left_ref_imgs', 'frame_stride', 'video_id')),
    dict(type='ConcatVideoReferences'),
...
data = dict(
    ...
    val=dict(
        type=dataset_type,
        odd_working_mode="agg",
        ann_file=data_root + 'annotations/imagenet_vid_val_with_odds.json',
        img_prefix=data_root + 'Data/VID',
        ref_img_sampler=dict(
            num_ref_imgs=14,
            frame_range=[-7, 7],
            method='test_global_odd'),
